...
public class DataSupport {
	...
	/**
	 * The limit of the count of record int the table.
	 */
	...
	public synchronized boolean save() {
		try {
		// ----------
			int count = count(this.getClass());
			if(maxRecord > 0 && count >= maxRecord)
			{
				findFirst(this.getClass()).delete();
			}
			LogUtil.print(LogUtil.LogPriorityEnum.CORE_LOG_PRI_INFO, "SAVE -> " + this.toString());
		// -----------
			saveThrows();
			return true;
		} catch (Exception e) {
	...
	public synchronized void maxRecord(int maxRecord)
	{
	 ...
	 */
	private synchronized void saveThrows() {
		SQLiteDatabase db = Connector.getDatabase();
		db.beginTransaction();
		try {
			SaveHandler saveHandler = new SaveHandler(db);
